...
**Дата:** 2026-04-22 (обновлено во 2-й итерации)
**Автор сессии:** E1
...
## 0. Что нового во 2-й итерации

1. **Все уровни теперь 100%-проходимы.** Написан BFS-валидатор
   `apps/web/scripts/checkLevels.mjs`, который проверяет возможность собрать
   ВСЕ звёзды и достичь финиша. Текущие уровни дают минимум 4/4/6/9/16
   свайпов для 100%.
2. **Система персонажей.** `core/characters/registry.ts` описывает
   персонажей с условиями разблокировки по `total stars`. MVP: 1 реальный
   (`blob`) + 3 placeholder'а. Новый персонаж добавляется одной записью в
   registry.
3. **Character picker** в главном меню — горизонтальный snap-scroll со
   значками персонажей.
4. **Визуальные улучшения движка:**
   - `ParticleEffects.ts` — `emitStarBurst` (звёзды) и `emitConfetti` (финиш).
   - Звезда теперь — `core + halo-disc` с эмиссивом.
   - Портал — трёхслойный (`outer torus + inner torus + halo disc`) с пульсом.
   - Player squash&stretch на скольжении (без нового скелета — только через
     `mesh.scaling`).
5. **Детальный рыночный ресёрч** в `RESEARCH.md` с 7 альтернативными
   game-концептами и рекомендациями.

---

## 1. Что сделано за обе сессии
...
### Уровни (5 hand-crafted, 100% проходимы)
| ID | Название      | Сетка   | Мин. свайпов 100% | Описание                    |
|----|---------------|---------|-------------------|-----------------------------|
| 1  | Первые шаги   | 11×11   | 6                 | Интро, 4 звезды в углах     |
| 2  | Коридоры      | 11×11   | 4                 | Змейка, 3 звезды             |
| 3  | Перекрёсток   | 11×11   | 6                 | Ветвящийся лабиринт          |
| 4  | Змейка        | 11×11   | 9                 | Серпантин с зажимами         |
| 5  | Спираль       | 11×11   | 16                | Концентрические коридоры     |

Остальные 6–60 в меню показаны как «Скоро» с замком. Чтобы добавить уровень,
положите JSON в `apps/web/src/features/game/levels/`, добавьте его в
`registry.ts` и **запустите `node scripts/checkLevels.mjs` для валидации**.

### Персонажи (character registry)
| ID           | Имя            | Unlock              | Ассет         |
|--------------|----------------|---------------------|---------------|
| blob         | Блоб           | always              | `blob.glb`    |
| coin-cat     | Кот-копилка    | ★ ≥ 9               | placeholder → blob |
| paper-fox    | Лис-оригами    | ★ ≥ 27              | placeholder → blob |
| cloud-ghost  | Облачный дух   | ★ ≥ 60              | placeholder → blob |

Placeholder'ы используют blob.glb. Чтобы подключить реальный ассет —
кинуть `.glb` в `shared/assets/models/` и заменить `modelUrl: null` на
`import`.
...
- `node scripts/checkLevels.mjs` — все 5 уровней 100%-проходимы.

### Скриншотами через Playwright (headless)
| Проверка                                    | Результат |
|---------------------------------------------|-----------|
| Меню рендерится                             | ✅         |
| CharacterPicker отрисован, 4 карточки        | ✅         |
...
| Babylon-сцена рендерится (blob + звёзды)     | ✅         |
| Top-down камера                             | ✅         |
| Star-halo disc вокруг звёзд                 | ✅         |
| Portal с двумя торами + halo                | ✅         |
| Keyboard-fallback: свайп-стрелки работают   | ✅         |
| Сбор звезды: HUD обновляется `0/N → K/N`    | ✅         |
| Particle burst при сборе звезды             | ✅         |
| Достижение портала → оверлей «пройден»      | ✅         |
| Confetti на finish                           | ✅         |
| Рейтинг 1/2/3 по %                          | ✅         |
| 100% прохождение уровня (из валидатора)      | ✅         |
